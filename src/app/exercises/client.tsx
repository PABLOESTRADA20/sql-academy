...
const DEFAULT_SCHEMA = "productos (id INT, nombre TEXT, precio DECIMAL, stock INT, categoria TEXT)\nusuarios (id INT, nombre TEXT, email TEXT, edad INT, ciudad TEXT)";
const DEFAULT_DATA = "INSERT INTO productos VALUES (1, 'Laptop Pro', 2999.99, 10, 'Electrónicos'), (2, 'Monitor 4K', 899.99, 25, 'Electrónicos'), (3, 'Teclado Mecánico', 149.99, 50, 'Accesorios');\nINSERT INTO usuarios VALUES (1, 'Ana García', 'ana@email.com', 28, 'Madrid'), (2, 'Carlos López', 'carlos@email.com', 32, 'Barcelona');";
...
function ExerciseEditor({ exercise }: { exercise: { id: string; tableSchema?: string; initialData?: string } }) {
  ...
  const getOrCreateDb = async () => {
    ...
    try {
      const schema = exercise.tableSchema || DEFAULT_SCHEMA;
      const data = exercise.initialData || DEFAULT_DATA;
      const db = await createExerciseDatabase(schema, data);
      dbRef.current = db;
      return db;
    } finally {
    ...
    <div className="mt-4 space-y-3">
      <div className="rounded-lg border border-border/50 bg-muted/20">
        <div className="flex items-center justify-between border-b border-border/50 px-3 py-2">
          ...
          <Button size="sm" className="h-7 text-xs" onClick={handleExecute} disabled={executing || initializing} aria-label="Ejecutar consulta SQL">
            {executing ? (
        ...
        <textarea
          ...
          className="w-full min-h-[100px] bg-[#0B1121] text-gray-100 font-mono text-sm p-3 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-primary/30 resize-y leading-relaxed"
          spellCheck={false}
      ...
      {result && (
        <div className="rounded-lg border border-border/50 overflow-hidden" role="region" aria-label="Resultado">
          {result.error ? (
            <div className="flex items-start gap-2 bg-red-500/10 p-3 text-sm text-red-500" role="alert">
...
export function ExercisesClient() {
  ...
  return (
    <section aria-label="Lista de ejercicios SQL">
          ...
          <TabsContent key={tab.value} value={tab.value} className="m-0">
            <div className="space-y-4">
              {filteredExercises(tab.value).map((exercise, idx) => (
                <Card key={exercise.id} className={cn(completed.has(exercise.id) ? "border-green-500/50" : "border-border/50")}>
                  <CardHeader className="p-4 pb-2">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-3">
                        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-medium" aria-hidden="true">
                          {idx + 1}
                        </span>
                        <div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button
                          ...
                          aria-label={completed.has(exercise.id) ? "Marcar como no completado" : "Marcar como completado"}
                        >
