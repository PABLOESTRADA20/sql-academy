...
export async function generateStaticParams() {
  return postgresqlLessons.lessons.map((lesson) => ({
    slug: lesson.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const lesson = postgresqlLessons.lessons.find((l) => l.slug === slug);
  if (!lesson) return {};
  return {
    title: lesson.title,
...
export default async function PostgresqlLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const lesson = postgresqlLessons.lessons.find((l) => l.slug === slug);
  if (!lesson) notFound();

  const currentIndex = postgresqlLessons.lessons.findIndex((l) => l.slug === slug);
  const prevLesson = currentIndex > 0 ? postgresqlLessons.lessons[currentIndex - 1] : null;
  const nextLesson = currentIndex < postgresqlLessons.lessons.length - 1 ? postgresqlLessons.lessons[currentIndex + 1] : null;
  ...
  const renderContent = (content: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("```")) {
        ...
        if (match) {
          return <CodeBlock key={i} code={match[2].trim()} language={match[1] || "sql"} />;
        }
    ...
    <article className="max-w-none">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          <Badge variant="advanced">PostgreSQL</Badge>
          <span className="text-sm text-muted-foreground">20 min de lectura</span>
        </div>
      ...
      <div className="mt-12 flex items-center justify-between border-t border-border/50 pt-6">
        <div>
          {prevLesson && (
            <Link
              href={`/postgresql/${prevLesson.slug}`}
        ...
        <div>
          {nextLesson && (
            <Link
              href={`/postgresql/${nextLesson.slug}`}
              className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              {nextLesson.title}
