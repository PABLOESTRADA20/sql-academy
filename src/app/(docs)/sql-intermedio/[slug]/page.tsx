...
export async function generateStaticParams() {
  return sqlIntermedio.lessons.map((lesson) => ({
    slug: lesson.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const lesson = sqlIntermedio.lessons.find((l) => l.slug === slug);
  if (!lesson) return {};
  return {
    title: lesson.title,
...
export default async function SqlIntermedioLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const lesson = sqlIntermedio.lessons.find((l) => l.slug === slug);
  if (!lesson) notFound();

  const currentIndex = sqlIntermedio.lessons.findIndex((l) => l.slug === slug);
  const prevLesson = currentIndex > 0 ? sqlIntermedio.lessons[currentIndex - 1] : null;
  const nextLesson = currentIndex < sqlIntermedio.lessons.length - 1 ? sqlIntermedio.lessons[currentIndex + 1] : null;
    ...
    <article className="max-w-none">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-4">
          <Badge variant="intermediate">Intermedio</Badge>
          <span className="text-sm text-muted-foreground">15 min de lectura</span>
        </div>
      ...
      <div className="mt-12 flex items-center justify-between border-t border-border/50 pt-6">
        <div>
          {prevLesson && (
            <Link
              href={`/sql-intermedio/${prevLesson.slug}`}
        ...
        <div>
          {nextLesson && (
            <Link
              href={`/sql-intermedio/${nextLesson.slug}`}
              className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              {nextLesson.title}
